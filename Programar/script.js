...
let autoGenerateMode = false;   // ← true when coming from crear-app
let dcxAppSvgIcon   = '';       // ← SVG icon generated by AI
let dcxAppIconName  = '';       // ← App name from AI
...
const debouncedPreview = debounce(() => {
    ...
    // Build favicon data URI from SVG icon if available
    const faviconHref = dcxAppSvgIcon
        ? 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(dcxAppSvgIcon)))
        : '';
    const doc = `<!DOCTYPE html><html><head><meta charset="UTF-8"><link rel="shortcut icon" href="${faviconHref}" type="image/x-icon"><style>${css}</style></head><body>${html}<script>
...
function switchTab(fileType) {
    ...
    ['html','js','css','svg'].forEach(l=>document.getElementById(l+'EditorWrapper')?.classList.remove('active'));
    ...
    if (fileType==='preview') {
        ...
        const sw=document.getElementById('svgEditorWrapper');if(sw)sw.classList.add('active');
        const ed=document.getElementById('svgEditor');
        if(ed){ed.focus();updateStatusLang('svg');}
    } else {
...
async function saveToFirebase(html, css, js) {
    ...
    try {
        ...
        const updated = (snap.data()?.proyectos || []).map(p =>
            p.id === dcxProjectId ? { ...p, html, css, js, svgIcon: dcxAppSvgIcon||p.svgIcon||'', updatedAt: now } : p
        );
...
async function generateCode(prompt){
    ...
    try{
        ...
        const fullPrompt=`${context}Solicitud: ${prompt}\n\nGenera una app web completa con HTML, CSS, JS, un icono SVG representativo y un nombre creativo para la app.`;
        ...
        // API now uses Gemini JSON mode — parse is always valid
        let parsed;
        try{parsed=JSON.parse(rawText);}
        catch(e){const m=rawText.match(/\{[\s\S]*\}/);if(m)try{parsed=JSON.parse(m[0]);}catch(_){}if(!parsed)throw new Error('La IA no devolvió JSON válido.');}
        ...
        if(autoGenerateMode){
            ...
            showGenerationResultCard(parsed);
        } else {
            updatePreview();
...
// ── Result card shown after auto-generation ─────────────────────────────────
function showGenerationResultCard(parsed){
    ...
    document.getElementById('dcxCardContinue').addEventListener('click',async()=>{
        ...
        if(dcxAppSvgIcon&&dcxProjectId&&dcxUid){
            try{
                ...
                const ref=doc(dcxDb,'proyectos',dcxUid);const snap=await getDoc(ref);
                if(snap.exists()){
                    const now=new Date().toISOString();
                    const updated=(snap.data()?.proyectos||[]).map(p=>
                        p.id===dcxProjectId?{...p,svgIcon:dcxAppSvgIcon,name:dcxAppIconName||p.name,updatedAt:now}:p);
                    await updateDoc(ref,{proyectos:updated});
                }
            }catch(e){console.warn('[DCX] saveIcon failed:',e);}
        }
        updatePreview();
        ...
        saveToLocal();switchTab('preview');
        incrementAgentUsage();
        // Update project name display if AI gave a name
        if(dcxAppIconName){const ns=document.getElementById('projectName');if(ns)ns.textContent=dcxAppIconName;}
        // Clear autogen storage
        localStorage.removeItem('dcx_autogen_name');
        localStorage.removeItem('dcx_autogen_desc');
    });
}

// ── Auto-generate mode: lock UI to chat-only, then auto-send prompt ─────────
function initAutoGenerateMode(){
...
document.addEventListener('DOMContentLoaded', async function init(){
    ...
    // Read project params from URL (?pid=... ?generated=true ?autoGenerate=true)
    const urlParams = new URLSearchParams(window.location.search);
    dcxProjectId = urlParams.get('pid') || null;
    const isGenerated    = urlParams.get('generated')    === 'true';
    const isAutoGenerate = urlParams.get('autoGenerate') === 'true';
    ...
    if (isAutoGenerate) {
        history.replaceState(null, '', '/Programar/' + (dcxProjectId ? '?pid=' + dcxProjectId : ''));
        initAutoGenerateMode();
    } else {
        switchTab('html');
    }
